...
public class DepartmentController {
	...
	@CrossOrigin
	@ResponseBody
	public ResponseEntity<?> adddepartment(@RequestBody DepartmentDetails departmentdetail) {
		HttpHeaders headers = new HttpHeaders();

		try {
			return ResponseEntity.status(HttpStatus.CREATED).headers(headers)
					.body(departmentservice.adddepartment(departmentdetail));
		} catch (Exception e) {
	...
	public ResponseEntity<?> findalldepartment() {
		...
		try {
			return ResponseEntity.status(HttpStatus.CREATED).headers(headers)
					.body(departmentservice.findalldepartment());
		} catch (Exception e) {
			headers.add("Message", "false");
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).headers(headers)
					.body("Failed to add the user");
